...
def nav():
    ...
    while Xn != goal[0] and Yn != goal[1]:
        for i in range(0,len(path)-1):
            v, rz = plot_curve(path[i][0],path[i][1],theta[i],spin[i+1][0],spin[i+1][1],path[i+1][0],path[i+1][1])
            if i >= 10 and abs(rz) > 0.4:
                v = 0.10*260 #0.07
            vel_msg.linear.x = v/250 #260
            vel_msg.angular.z = rz/3.35 #3.5 #3.4
            pub.publish(vel_msg)
            time.sleep(1.9) #2
